...
/*	var icoPath = ""
	icoPath, _ = os.Getwd()
	icoPath = icoPath+"\\Ico.ico"
	icoPath = strings.Replace(icoPath,"\\","\\\\",-1)*/
	// 1.先判断配置文件是否存在
	userDir, err := user.Current()
	if err != nil {
		fmt.Println(err)
	}
	configPath := userDir.HomeDir+"\\AppData\\Roaming\\lsblj\\lsblj.config"
	pathTemp := userDir.HomeDir+"\\AppData\\Roaming\\lsblj"
	...
	if !isTrue {
		...
		var data = ""
		if os.Args==nil {
			data = baseDeEncode(strings.Replace(os.Args[1],"lsblj://","",-1))
		}else{
			paramNumber = baseDeEncode(strings.Replace(os.Args[1],"lsblj://","",-1))
			data = baseDeEncode(strings.Replace(os.Args[1],"lsblj://","",-1))
		}
		p := &Param{}
...
func createRegisterTable() {
	...
	if file,err := os.Create("./install.reg"); err!=nil{
		...
		data += "[HKEY_LOCAL_MACHINE\\SOFTWARE\\Classes\\lsblj]\n"
		data += "@=\"lsblj\"\n"
		data += "\"URL Protocol\"=\""
		data += strings.Replace(strings.Replace(str, "\\", "\\\\", -1), "\\\\baoleiji.exe", "", -1)+"\"\n\n"
		data += "[HKEY_LOCAL_MACHINE\\SOFTWARE\\Classes\\lsblj\\DefaultIcon]\n"
		data += "@=\"%SystemRoot%\\system32\\url.dll,0\"\n\n"
		data += "[HKEY_LOCAL_MACHINE\\SOFTWARE\\Classes\\lsblj\\Shell]\n"
		data += "[HKEY_LOCAL_MACHINE\\SOFTWARE\\Classes\\lsblj\\Shell\\open]\n"
		data += "[HKEY_LOCAL_MACHINE\\SOFTWARE\\Classes\\lsblj\\Shell\\open]\n"
		data += "[HKEY_LOCAL_MACHINE\\SOFTWARE\\Classes\\lsblj\\Shell\\open\\command]\n"
		data += "@=\"\\\""
